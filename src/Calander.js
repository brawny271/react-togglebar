...
import { format, startOfMonth, addMonths, subMonths, startOfWeek, addDays } from "date-fns";

const Calander = ({ events }) => {
  ...
  const renderCalendar = () => {
    ...
    return daysInMonth.map((day, index) => {
      ...
      const isCurrentDate = format(day, "yyyy-MM-dd") === format(currentDate, "yyyy-MM-dd");
      const hasEvents = dayEvents.length > 0;

      return (
        <div
          key={index}
          className={`day ${format(day, "MM") !== format(currentMonth, "MM") ? "text-gray-400" : ""} ${isCurrentDate ? "bg-gray-200" : ""} ${hasEvents ? "rounded-full bg-gray-400" : ""}`}
          onMouseEnter={() => handleMouseEnter(day)}
          onMouseLeave={handleMouseLeave}
        >
          {format(day, "d")}
          {dayEvents.length > 0 && hoveredDate && format(hoveredDate, "yyyy-MM-dd") === format(day, "yyyy-MM-dd") && (
